...
newCheckBox = (name='test',func = () => {console.log("Test: Active")},func2 = () => {console.log("Test: Deactivated")},args=0) => {
...
function clickAble(id,func,func2,args){
    ...
    if(checkboxes.children[0].classList.value.split(' ').includes("checked")){
        ...
        func(args)
        CFVariables['checkbox_active'][id]=0
        return false;
    }
    ...
    func2(args)
    CFVariables['checkbox_active'][id]=1
    return true;
}
...
getCardList = async (id) => {
    let url = `https://steamcommunity.com/market/search/render/?query=appid%3A753&search?q=&category_753_Game%5B%5D=tag_app_${id}&category_753_cardborder%5B%5D=tag_cardborder_0&category_753_item_class%5B%5D=tag_item_class_2&start=0&count=50&norender=1`
    await getRequest(url).then((response) => {
        let data = response.results
        let card_hashnames = []
        for(let i=0;i<data.length;i++){
            card_hashnames.push(data[i].hash_name)
        }
    })
}

getCardPrice = async (hashname,currency=1) =>{
    let url = `https://steamcommunity.com/market/priceoverview/?currency=${currency}&appid=753&market_hash_name=${hashname}`
...
function analize(){
    /*if(!CFVariables['intervalActiveIndex']){
        setInterval()
    }*/
    ...
    for(let i=CFVariables['custom_filter_index'];i<review_a.length;i++){
